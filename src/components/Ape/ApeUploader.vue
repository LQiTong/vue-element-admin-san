<template>
  <div>
    <el-upload class="ape-uploader" :class="{'ape-uploader-image':isImage, 'ape-uploader-disabled':disabledUpload}" name="file" :action="uploadToken.host" :headers="uploadHeaders" :data="uploadData" :on-preview="handlePictureCardPreview" :before-upload="beforeUpload" :on-success="afterUpload" :on-remove="removeUpload" :list-type="listType" :on-error="onError" :file-list="uploadFileList" :limit="limit" :on-exceed="onExceed">
      <el-button v-if="listType == 'text' && !disabledUpload" size="small" type="primary">点击上传</el-button>
      <i v-if="listType == 'picture-card' && !disabledUpload" class="el-icon-plus" />
    </el-upload>
...
export default {
  ...
  methods: {
    ...
    async beforeUpload(file) {
      // console.log(file)
      const checkType = this.allowTypeList.indexOf(file.type)
      const checkSize = file.size > this.allowSize
    ...
    formatFileList(fileList) {
      ...
      for (var i = 0; i < fileList.length; i++) {
        if (fileList[i].response === undefined) {
          ...
          const resData = fileList[i].response.data[0]
          const item = {
            id: resData.id,
            name: resData.title,
            path: resData.path,
